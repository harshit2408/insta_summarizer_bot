...
{{
  ...
  "category":      string,  // exactly one of: {_CATEGORY_LIST}
...
def build_messages(
    *,
    ...
    Unknown variants fall back to the default, so an mistyped env var can
    never crash the Lambda — it just runs with v1.
    """
    system = _VARIANTS.get(variant, _VARIANTS[DEFAULT_VARIANT])
